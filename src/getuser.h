...
void init_primary_textures_getuser(SDL_Renderer* renderer){
    ...
    for(int i=0;i<buttons_count_get_user;i++){
        blue_captions_get_user[i] = getTextTexture(renderer, "./assets/fonts/liber.ttf", buttons_get_user[i].caption, blue);
        black_captions_get_user[i] = getTextTexture(renderer, "./assets/fonts/liber.ttf", buttons_get_user[i].caption, black);
    }
    user_name_rect.x = 400;
    user_name_rect.y = 400;

    caption_get_user = getTextTexture(renderer, "./assets/fonts/liber.ttf","Enter your name: ",black);
...
void create_new_username_texture(SDL_Renderer* renderer, char* name){
    ...
    user_name_texture = getTextTexture(renderer, "./assets/fonts/liber.ttf", name, black);
