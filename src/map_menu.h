...
void init_buttons_map_menu(SDL_Renderer* renderer){
    ...
    for(int i=0;i<maps_count+2;i++){
        blue_textures_map_menu[i] = getTextTexture(renderer, "./assets/fonts/liber.ttf", buttons_map_menu[i].caption, blue);
        black_textures_map_menu[i] = getTextTexture(renderer, "./assets/fonts/liber.ttf", buttons_map_menu[i].caption, black);
    }
    for(int i=0;i<3;i++){
...
void draw_map_menu(SDL_Renderer* renderer){
    ...
    stringRGBA(renderer, 10, 720, "New map players:", 0,0,0,255);
    for(int i=0;i<3;i++){
        rect.x = buttons_players_count[i].x;
        rect.y = buttons_players_count[i].y;
        rect.h = buttons_players_count[i].h;
        rect.w = buttons_players_count[i].w;
        char number[3] = "";
        sprintf(number, "%d", i+2);
        SDL_RenderCopyEx(renderer, btn_main,NULL, &rect, 0,NULL, SDL_FLIP_NONE);
        if((current_mouse_x >= rect.x && current_mouse_x - rect.x <= rect.w 
            && current_mouse_y >= rect.y && current_mouse_y - rect.y <= rect.h)
        || (selected_player == i+2) ){
            stringRGBA(renderer, rect.x + 20, rect.y + 20, number, 0,0,255,255);
        }else{
            stringRGBA(renderer, rect.x + 20, rect.y + 20, number, 0,0,0,255);
        }
    }
    draw_map(renderer);
    //draw_camps(renderer);
...
void detect_click_map_menu(SDL_Renderer* renderer, int x, int y, int count){
    ...
    for(int i=0;i<3;i++){
        if(x - buttons_players_count[i].x <= buttons_players_count[i].w && x >= buttons_players_count[i].x
        && y - buttons_players_count[i].y <= buttons_players_count[i].h && y >= buttons_players_count[i].y){
            selected_player = i+2;
        }
    }
}
