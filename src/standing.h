...
void init_textures_standing(SDL_Renderer* renderer){
    ...
    back_button_standing[0] = getTextTexture(renderer, "./assets/fonts/liber.ttf", "Back", black);
    back_button_standing[1] = getTextTexture(renderer, "./assets/fonts/liber.ttf", "Back", blue);
    sort_scores();
    ...
    for(int i=0;i<users_to_show;i++){
        standing_names[i] = getTextTexture(renderer, "./assets/fonts/liber.ttf", user_names[i],i%2?black:blue);
        char score_temp[20] = "";
        sprintf(score_temp, "%d", users_scores[i]);
        standing_scores[i] = getTextTexture(renderer, "./assets/fonts/liber.ttf", score_temp,i%2?black:blue);
    }
    standing_names[users_to_show] = getTextTexture(renderer, "./assets/fonts/liber.ttf", "Name",black);
    standing_scores[users_to_show] = getTextTexture(renderer, "./assets/fonts/liber.ttf", "Score",black);
}
